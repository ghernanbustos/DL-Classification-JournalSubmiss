...
load ('Sinc_25dB_Trai.nmat'); % 5000x256

load ('Sinc_25dB_Test.mat');  % 5000x256
load ('Sinc_20dB_Test.mat');  % 5000x256
load ('Sinc_15dB_Test.mat');  % 5000x256
load ('Sinc_10dB_Test.mat');  % 5000x256
...
% -------------------------------------------------------------------------
%---------- Load New DataSet for testing, this Dataset is ordered by label
%not shuffled like the one used during training

TrueLabelData = categorical(Target_LabelNumbered'); % transpose the original label mat. Not Shuffled

YTest_25dB = classify(net,TestMat_Sinc_25dB, SequencePaddingDirection="left");
acc_25dB = mean(YTest_25dB == TrueLabelData) % comparing the predicted labels with the true.

YTest_20dB = classify(net,TestMat_Sinc_20dB, SequencePaddingDirection="left");
acc_20dB = mean(YTest_20dB == TrueLabelData)

YTest_15dB = classify(net,TestMat_Sinc_15dB, SequencePaddingDirection="left");
acc_15dB = mean(YTest_15dB == TrueLabelData)

YTest_10dB = classify(net,TestMat_Sinc_10dB, SequencePaddingDirection="left");
