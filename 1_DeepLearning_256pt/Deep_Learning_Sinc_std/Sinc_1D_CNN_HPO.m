...
for nIteration = 1 : size(TrainMat_Sinc_25dB, 1)
    TrainMat_Sinc_25dB(nIteration,:) = rescale(TrainMat_Sinc_25dB(nIteration,:),-1,1); 
end

for nIteration = 1 : size(TestMat_Sinc_25dB, 1)
    TestMat_Sinc_25dB(nIteration,:) = rescale(TestMat_Sinc_25dB(nIteration,:),-1,1); 
end

for nIteration = 1 : size(TestMat_Sinc_20dB, 1)
    TestMat_Sinc_20dB(nIteration,:) = rescale(TestMat_Sinc_20dB(nIteration,:),-1,1); 
end

for nIteration = 1 : size(TestMat_Sinc_15dB, 1)
    TestMat_Sinc_15dB(nIteration,:) = rescale(TestMat_Sinc_15dB(nIteration,:),-1,1); 
end

for nIteration = 1 : size(TestMat_Sinc_10dB, 1)
    TestMat_Sinc_10dB(nIteration,:) = rescale(TestMat_Sinc_10dB(nIteration,:),-1,1); 
end

% Numerical Data label and Categorical label with type of Signal
% deformation No deformation, Sincian and LPB filter.
% Label {1 = NoDef, 2 = Sinc1, 3 = Sinc2, 4 = Pb1, 5 = Pb2}

% ------- Prepare DataSet from Ordered Balanced to randomly shuffle  
%               Generate a random permutation of indices
[numObservations,numLabels] = size(TrainMat_Sinc_25dB);
...
XTrain = TrainMat_Sinc_25dB_shuffle(1:idxTrain,:);
TTrain = Target_LabelNumbered_shuffle(1:idxTrain,:);

XValidation = TrainMat_Sinc_25dB_shuffle   (idxTrain+1 :idxTrain + idxValidation , :);
TValidation = Target_LabelNumbered_shuffle (idxTrain+1 :idxTrain + idxValidation, :);

XTest = TrainMat_Sinc_25dB_shuffle  (idxTrain + idxValidation + 1 : end, :);
...
YTest_25dB = classify(BestAnn.trainedNet,TestMat_Sinc_25dB, SequencePaddingDirection="left");
acc_25dB = mean(YTest_25dB == TrueLabelData) % comparing the predicted labels with the true.

YTest_20dB = classify(BestAnn.trainedNet,TestMat_Sinc_20dB, SequencePaddingDirection="left");
acc_20dB = mean(YTest_20dB == TrueLabelData)

YTest_15dB = classify(BestAnn.trainedNet,TestMat_Sinc_15dB, SequencePaddingDirection="left");
acc_15dB = mean(YTest_15dB == TrueLabelData)

YTest_10dB = classify(BestAnn.trainedNet,TestMat_Sinc_10dB, SequencePaddingDirection="left");
acc_10dB = mean(YTest_10dB == TrueLabelData)

% ---------------------- Save Acc% Data file ------------------------------

% Open a text file for writing
fileID = fopen('Acc_Ann_Sinc_STD.txt', 'w');
...
% 
% Save WorkOut
save('SincWorkspaceSTD'); % inputName
