...
% -------------------------------------------------------------------------
for nIteration = 1 : size(TrainMat_Chirp_25dB, 1)
    TrainMat_Chirp_25dB(nIteration,:) = rescale(TrainMat_Chirp_25dB(nIteration,:),-1,1); 
end

for nIteration = 1 : size(TestMat_Chirp_25dB, 1)
    TestMat_Chirp_25dB(nIteration,:) = rescale(TestMat_Chirp_25dB(nIteration,:),-1,1); 
end

for nIteration = 1 : size(TestMat_Chirp_20dB, 1)
    TestMat_Chirp_20dB(nIteration,:) = rescale(TestMat_Chirp_20dB(nIteration,:),-1,1); 
end

for nIteration = 1 : size(TestMat_Chirp_15dB, 1)
    TestMat_Chirp_15dB(nIteration,:) = rescale(TestMat_Chirp_15dB(nIteration,:),-1,1); 
end

for nIteration = 1 : size(TestMat_Chirp_10dB, 1)
    TestMat_Chirp_10dB(nIteration,:) = rescale(TestMat_Chirp_10dB(nIteration,:),-1,1); 
end
...
fileID = fopen('Acc_Ann_Chirp_STD.txt', 'w');
...
% 
% Save WorkOut
save('ChirpWorkspaceSTD'); % inputName
