...
load ('Gauss_25dB_Train.mat'); % 5000x1024

load ('Gauss_25dB_Test.mat');  % 5000x1024
load ('Gauss_20dB_Test.mat');  % 5000x1024
load ('Gauss_15dB_Test.mat');  % 5000x1024
load ('Gauss_10dB_Test.mat');  % 5000x1024
load ('Target_LabelNumbered.mat');


for nIteration = 1 : size(TrainMat_Gauss_25dB, 1)
    TrainMat_Gauss_25dB(nIteration,:) = rescale(TrainMat_Gauss_25dB(nIteration,:),-1,1); 
end

for nIteration = 1 : size(TestMat_Gauss_25dB, 1)
    TestMat_Gauss_25dB(nIteration,:) = rescale(TestMat_Gauss_25dB(nIteration,:),-1,1); 
end

for nIteration = 1 : size(TestMat_Gauss_20dB, 1)
    TestMat_Gauss_20dB(nIteration,:) = rescale(TestMat_Gauss_20dB(nIteration,:),-1,1); 
end

for nIteration = 1 : size(TestMat_Gauss_15dB, 1)
    TestMat_Gauss_15dB(nIteration,:) = rescale(TestMat_Gauss_15dB(nIteration,:),-1,1); 
end

for nIteration = 1 : size(TestMat_Gauss_10dB, 1)
    TestMat_Gauss_10dB(nIteration,:) = rescale(TestMat_Gauss_10dB(nIteration,:),-1,1); 
end
...
YTest_25dB = classify(BestAnn.trainedNet,TestMat_Gauss_25dB, SequencePaddingDirection="left");
acc_25dB = mean(YTest_25dB == TrueLabelData) % comparing the predicted labels with the true.

YTest_20dB = classify(BestAnn.trainedNet,TestMat_Gauss_20dB, SequencePaddingDirection="left");
acc_20dB = mean(YTest_20dB == TrueLabelData)

YTest_15dB = classify(BestAnn.trainedNet,TestMat_Gauss_15dB, SequencePaddingDirection="left");
acc_15dB = mean(YTest_15dB == TrueLabelData)

YTest_10dB = classify(BestAnn.trainedNet,TestMat_Gauss_10dB, SequencePaddingDirection="left");
acc_10dB = mean(YTest_10dB == TrueLabelData)

% ---------------------- Save Acc% Data file ------------------------------

% Open a text file for writing
fileID = fopen('Acc_Ann_Gauss_STD.txt', 'w');
...
% 
% Save WorkOut
save('GaussWorkspaceSTD'); % inputName
